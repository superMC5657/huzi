...
> **路径基准说明**：本文所有命令默认 cwd=仓库根；`huzc/` 内文档（`huzc/README.md`、`huzc/docs/USAGE.md` 等）默认 cwd=`huzc`，两者路径差一个 `huzc/` 前缀，请勿混用。

## 四模块一览

| 模块 | 路径 | 说明 |
|------|------|------|
| 编译器与工具链 | `huzc/` | 官方参考编译器（Rust workspace + LLVM 后端），含 `crates/`、`docs/`、`test/cases/`、`test.sh` |
...
- **用户指南与语法速查**：[`huzc/docs/USAGE.md`](huzc/docs/USAGE.md)（cwd=`huzc`）
- **编译器工程说明**：[`huzc/README.md`](huzc/README.md)（cwd=`huzc`）
- **项目完成状态**：[`huzc/docs/STATUS.md`](huzc/docs/STATUS.md)
- **编码与重构约定**：[`huzc/AGENTS.md`](huzc/AGENTS.md)（cwd=`huzc`，含验证流程与提交约定）
