...
- `chan_send(ch: i64, msg: str) -> bool`: 发送消息；缓冲满时阻塞直至有空间；句柄为 null 返回 false（非 null 无效句柄未做校验，勿传垃圾值）。
...
- **注释保留范围说明**：当前版本 `huzc fmt` 为基于 AST 语法树的代码美化器（AST Pretty Printer）。`//` 与 `#` 行注释格式化后原样保留（整行注释按语句回插，行尾注释随语句拼接）；个别位置如结构体字段间可能整体偏移但不丢失。
