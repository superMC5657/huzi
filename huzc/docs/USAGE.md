...
依赖的 `version` 字符串原样保留，同时会被解析为版本需求 `VersionReq`（仅解析与“是否满足”匹配，不做求解）：
...
> **明确不做**：最高满足版本求解、`huzi.lock` 锁定文件、传递依赖合并。模块解析（`find`）与 `fetch` 仍按 `version` 原串精确路径行为（`vendor/<pkg>/<version>/` 与全局缓存），不受范围语义影响。
