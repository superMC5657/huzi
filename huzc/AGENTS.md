...
1. `cargo build --workspace` — **零错误、零警告**（cwd=`huzc`）。
2. 编译并运行 test/cases/（cwd=`huzc`；仓库根请用 `huzc/test/cases/`）下全部非交互示例,确认 exit=0;交互类示例(如 `10_guess_number_game.hz`)跳过(可直接运行 `bash test.sh`，cwd=`huzc`；仓库根请用 `bash huzc/test.sh`)。
3. `git diff` 复核:确认是纯搬移(删除行与新增行内容对应),没有夹带逻辑改动。
4. 新增 builtin 必须同提交同步文档:docs/guides/reference.md（cwd=`huzc`）补签名 → docs/STATUS.md 打勾 → test/cases/ 补示例,缺一即未通过。
