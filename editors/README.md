...
| L3 std import 感知 | `import std.json` 可解析到 `huzi-src`（读 `huzi.toml lib_entry`，参照 `modules.rs probe_entry_file`），`json::` 补全模块符号并可跳转到符号 | 规划中 |
| L4 Trait/impl 成员 | `Point.` 补全 impl 方法、`Trait::` 补全 trait 方法（读 `symbols.rs` trait/impl 表），语义高亮小幅扩展 | 规划中 |
